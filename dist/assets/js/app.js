...
function countdown(dataEnd) {
    ...
    function calculate() {
        ...
        if (timeRemaining >= 0) {
            ...
            document.querySelector("#timer__days").innerHTML = parseInt(
                days,
                10
            )
            document.querySelector("#timer__hours").innerHTML = (
                "0" + hours
            ).slice(-2)
...
function closeModal() {
    ...
    modal.querySelector(
        ".survey__modal-desc"
    ).textContent = `Мы хотим, чтобы свадьба прошла идеально не только для нас, но и для вас. Пожалуйста, заполните небольшой опросник по банкетному меню.`
...
function changeModal() {
    ...
    modal.querySelector(
        ".survey__modal-desc"
    ).textContent = `Заполните, пожалуйста, форму, чтобы мы знали, что у вас не получится прийти.`
    console.log(555)
...
ScrollReveal().reveal(
    ".conducting__img, .happy__img, .schedule__title, .dresscode__title, .dresscode__item, .requests__title",
    {
        origin: "top",
    }
)
ScrollReveal().reveal(
    ".conducting__text, .place__container, .happy__swiper, .survey__title, .survey__desc, .survey__btns, .timer__countdown",
    {
        origin: "bottom",
    }
)
ScrollReveal().reveal(
    ".date__img, .story__img, .schedule__box-left, .dresscode_color, .pictures__text",
    {
        origin: "left",
    }
)
ScrollReveal().reveal(
    ".date__text, .story__text, .schedule__box-right, .requests__row, .pictures__img",
    {
        origin: "right",
    }
)
